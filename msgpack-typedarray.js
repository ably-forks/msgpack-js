...
/**
 * decode an ArrayBuffer into a JS value
 * @param buffer: the ArrayBuffer
 * @param offset (optional): the offset into the ArrayBuffer to start decoding
 * @param length (optional): the number of bytes in the ArrayBuffer to decode
 * @param nodeBuffers (optional, default false): when decoding bin types, output
 *        as node Buffers instead of ArrayBuffers
 */
...
function Decoder(view, offset, nodeBuffers) {
	this.view = view;
	this.offset = offset || 0;
	this.nodeBuffers = nodeBuffers;
}
...
Decoder.prototype.bin = Decoder.prototype.buf = function(length) {
	...
	this.offset += length;
	return (this.nodeBuffers) ? Buffer.from(value) : value;
};
 ...
 *        as node Buffers instead of ArrayBuffers
 */
function decode(buffer, offset, length, nodeBuffers) {
	offset = offset || 0;
	length = length || (buffer.byteLength - offset);
	var view = new DataView(buffer, offset, length);
	var decoder = new Decoder(view, 0, nodeBuffers);
	var value = decoder.parse();
	if (decoder.offset !== length)
		throw new Error((length - decoder.offset) + " trailing bytes");
	return value;
}
...
/**
 * encode an JS value into an ArrayBuffer via a given DataView
 * @param value: the JS value to encode
